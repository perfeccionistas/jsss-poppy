...
// Drag state
let drag = {
  active: false, el: null, startDotIdx: null, lastDotIdx: null,
  ...
  function fadeNext() {
    ...
    if (i >= imgs.length) {
      setTimeout(() => {
        ...
        // Reveal Lookbook button and align it
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Harden lookbook UI immediately
  normalizeLookbookButton();
  hardenLookbookPanel();
  positionLookbook();
});
...
document.addEventListener('click', (e) => {
  ...
  // Only respond for .object-btn (A/B/C) or .lookbook-btn
  ...
  // Match the left column vertical span
...
// ===== LOOKBOOK hardening =====
function getLookbookBtn() {
  return document.querySelector('.lookbook-btn');
}
function normalizeLookbookButton() {
  const lb = getLookbookBtn();
  if (!lb) return;
  if (lb.classList.contains('object-btn')) lb.classList.remove('object-btn');
  ...
  lb.setAttribute('aria-label', 'Open Lookbook');
}
function hardenLookbookPanel() {
  const pnl = document.getElementById('popup-lookbook');
  if (!pnl) return;
  pnl.style.transformOrigin = 'right center';
  const minBtn = pnl.querySelector('.iv-minimize');
  if (minBtn) {
...
function positionLookbook(){
  ...
  lb.style.removeProperty('height');
