...
// ===== State =====
let occupied = new Set();       // dot indexes currently used by >= 1 item
let placed = [];                // array of placed DOM nodes
let selectedEl = null;          // current selection
let total = 0;                  // total price
let lastPlacedDotIdx = null;    // used by "Add"
let anchorDot = null;           // shared starting dot for A/B/C
...
function fillGrid() {
  ...
  // concatenation avoids backtick-escaping issues in some CMS
...
// ignores occupancy (used for dragging to ANY dot)
...
window.addEventListener("resize", function(){
  if (dotGrid.style.display === "grid") {
    fillGrid(); // anchorDot recalculated inside
  }
});

// ===== Popups (A/B/C and Lookbook share the same handler) =====
document.querySelectorAll('.object-btn').forEach(function(btn){
  btn.addEventListener('click', function(){
    const targetId = btn.getAttribute('data-popup');
    const targetPopup = document.getElementById(targetId);

    document.querySelectorAll('.popup-panel').forEach(function(p){
...
document.querySelectorAll('.popup-panel').forEach(function(panel){
  ...
  localButtons.forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      localVideos.forEach(function(vid, i){
...
function placePiece(type, atDot) {
  ...
  loader.crossOrigin = 'anonymous'; // enable alpha-picking
  loader.onload = function(){
    ...
    // Pivot: A = offset orbit; B/C = center pivot
    ...
    // Per-item selection (fallback; main picking below is alpha-aware)
...
placedLayer.addEventListener('click', function(e){
...
function buildPurchaseLink(){
  ...
  const parts = [];
  for (var k in counts) { parts.push('Piece ' + k + ': ' + counts[k]); }
...
document.querySelectorAll('.place-btn').forEach(function(btn){
  btn.addEventListener('click', function(e){
    ...
    // All A/B/C start on the SAME anchor dot
    if (anchorDot) {
...
bar.addEventListener('click', function(e){
  ...
  if (action === 'rotate') {
    ...
    if (type === 'B' || type === 'C') {
      // Snap to nearest 60 and advance by 60
...
function getImageCanvas(img) {
  ...
  _imgCanvasCache.set(key, { canvas: canvas, w: naturalW, h: naturalH });
  return canvas;
}
...
function pointToImagePixel(el, img, clientX, clientY) {
  ...
  const px = rect.left + to.ox;
  const py = rect.top  + to.oy;
  ...
  const pxImg = Math.floor(ex * scaleX);
  const pyImg = Math.floor(ey * scaleY);
...
function alphaHit(el, clientX, clientY) {
  ...
  const coords = pointToImagePixel(el, img, clientX, clientY);
  if (!coords) return false;

  try {
    const canvas = getImageCanvas(img);
    if (!canvas) return true;
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    const data = ctx.getImageData(coords.px, coords.py, 1, 1).data;
    return data[3] > 10;
  } catch (err) {
    _alphaHitCorsBroken = true;
    return true;
  }
}
...
document.addEventListener('click', function(e){
  ...
  const stack = document.elementsFromPoint(e.clientX, e.clientY)
    .filter(function(el){ return el.classList && el.classList.contains('placed-item'); });
  ...
  if (e.altKey) {
    ...
    selectPlaced(next, { bringToFront: false });
    return;
  }
  ...
  for (var k = 0; k < stack.length; k++) {
    var el = stack[k];
    if (alphaHit(el, e.clientX, e.clientY)) { picked = el; break; }
  }
  selectPlaced(picked || stack[0], { bringToFront: true });
}, true);

/* ==================== Drag to any dot (pointer events) ==================== */
function countItemsAtDot(idx) {
  let n = 0;
  for (const p of placed) if (Number(p.getAttribute('data-dot')) === idx) n++;
  return n;
}

function movePlacedToDot(el, dot) {
  const oldIdx = Number(el.getAttribute('data-dot'));
...
document.addEventListener('pointerdown', function(e){
  ...
  // Pick topmost opaque under cursor
  var picked = null;
  for (var k = 0; k < stack.length; k++) {
    var el = stack[k];
    if (alphaHit(el, e.clientX, e.clientY)) { picked = el; break; }
  ...
  drag.active = true;
  drag.el = picked;
  drag.startDotIdx = Number(picked.getAttribute('data-dot'));
  drag.lastDotIdx = drag.startDotIdx;
  drag.moved = false;
...
document.addEventListener('pointermove', function(e){
  ...
  // Use pageX/Y so they match our dot center coords (which include scroll)
  const dot = nearestDotTo(e.pageX, e.pageY);
  if (!dot) return;

  if (dot.idx !== drag.lastDotIdx) {
    movePlacedToDot(drag.el, dot);
    drag.lastDotIdx = dot.idx;
    drag.moved = true;
  }
});
...
function endDrag() {
  ...
  if (drag.moved) suppressNextClick = true; // avoid post-drag click selecting topmost
...
document.addEventListener('keydown', function(e){
  ...
  if (e.key === ']') {
    bringToFront(selectedEl);
  }

  if (e.key === '[') {
    const parent = selectedEl.parentNode;
    const prev = selectedEl.previousElementSibling;
    if (parent && prev) parent.insertBefore(selectedEl, prev);
  }

  if (e.key.toLowerCase() === 'm') {
    selectedEl.classList.toggle('blend-multiply');
  }
});

/* ===== LOOKBOOK minimize handler (open uses the shared .object-btn handler) ===== */
document.addEventListener('click', function(e){
