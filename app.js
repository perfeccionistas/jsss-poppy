...
const PIECES = {
  A: { src: "https://cdn.prod.website-files.com/67e7324b5a3b365fefc2fce4/68aca9e6fb38d895ba8a4ab8_Untitled-15.png", price: 16000},
  B: { src: "https://cdn.prod.website-files.com/67e7324b5a3b365fefc2fce4/68aca9e6bfa7fea95e7a5303_Untitled-17.png", price: 3500 },
  C: { src: "https://cdn.prod.website-files.com/67e7324b5a3b365fefc2fce4/68aca9e6d049292dd92fc22a_Untitled-16.png", price: 5500 }
};
...
// ===== Align LOOKBOOK pill to C (desktop) =====
function positionLookbook(){
  ...
  if (!lb || !cBtn) return;
  const _h = lb.offsetHeight; // force layout
  ...
  function fadeNext() {
    ...
    if (i >= imgs.length) {
      setTimeout(() => {
        ...
        // show Lookbook button now and align it
        const lb = document.querySelector('.lookbook-btn');
        if (lb){
...
// ===== Open popup (A/B/C + Lookbook) =====
document.addEventListener('click', (e) => {
  const btn = e.target.closest('.object-btn, .lookbook-btn');
  if (!btn) return;
  ...
  // Desktop: align top with button column (height handled by CSS)
  const isMobile = window.matchMedia('(max-width: 900px)').matches;
  if (!isMobile) {
    const col = document.querySelector('.object-button-column');
    const r = col?.getBoundingClientRect();
    const top = r ? r.top + window.scrollY : window.scrollY + 12;
  ...
  // grid visible but not clickable
...
function countItemsAtDot(idx) { let n = 0; for (const p of placed) if (Number(p.dataset.dot) === idx) n++; return n; }
