  ...
  function fadeNext() {
    ...
    if (i >= imgs.length) {
      setTimeout(() => {
        ...
        // >>> Reveal UI (Lookbook) and align it to C
...
// ===== Popups (A/B/C + LOOKBOOK because it's also .object-btn) =====
document.querySelectorAll('.object-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // small realign after layout shift
    positionLookbook();
  });
});
...
function getImageCanvas(img) {
  ...
  _imgCanvasCache.set(key, { canvas, w: naturalW, h: naturalH });
  return canvas;
}
...
function pointToImagePixel(el, img, clientX, clientY) {
  ...
  const px = rect.left + ox;
  const py = rect.top  + oy;
...
document.addEventListener('click', (e) => {
  ...
  const stack = document.elementsFromPoint(e.clientX, e.clientY)
    .filter(el => el.classList && el.classList.contains('placed-item'));
...
function countItemsAtDot(idx) {
  let n = 0;
  for (const p of placed) if (Number(p.dataset.dot) === idx) n++;
  return n;
}
...
document.addEventListener('pointerdown', (e) => {
  ...
  drag.active = true;
  drag.el = picked;
  drag.startDotIdx = Number(picked.dataset.dot);
...
/* ===== LOOKBOOK minimize handler (additive, non-breaking) ===== */
document.addEventListener('click', (e) => {
...
function positionLookbook(){
  ...
  // force a read so offsetHeight is valid if it was hidden
  const _h = lb.offsetHeight; // eslint-disable-line no-unused-vars
